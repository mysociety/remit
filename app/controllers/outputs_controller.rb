class OutputsController < ApplicationController
  ALLOWED_RESOURCE_TYPES = %w(study_impact dissemination publication)

  def create
    @study = Study.find(params[:study_id])
    resource_type = params[:output_type]
    if ALLOWED_RESOURCE_TYPES.include? resource_type
      self.send("create_#{resource_type}")
    else
  ...
  def create_study_impact
    ...
    StudyImpact.transaction do
      ...
      impact_type_ids.each do |key, impact_type_id|
        study_impact = StudyImpact.new(impact_type_id: impact_type_id,
      ...
      if @study_impacts.empty?
        ...
        redirect_to @study
      elsif @study_impacts.values.any? { |impact| impact.invalid? }
