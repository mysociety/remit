...
RSpec.describe "public_activity/study/_research_manager_id_changed.html.erb", type: :view do
  # rubocop:enable Metrics/LineLength
  let(:partial) { "public_activity/study/research_manager_id_changed" }
  let(:study) { FactoryGirl.create(:study) }
  let(:user) { FactoryGirl.create(:user) }
  let(:user2) { FactoryGirl.create(:user) }
  let(:activity_without_owner) do
    study.create_activity :research_manager_id_changed,
  ...
  let(:expected_title) { "Research manager changed" }
  ...
  context "when the user was removed" do
    ...
    it "sets the title to say that they were removed" do
      expected_text = "Research manager removed by #{user.name}"
